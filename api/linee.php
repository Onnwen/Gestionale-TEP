...
$extraQuery = "";
if (!is_null($id)) {
    $extraQuery = " WHERE Linee.idLinea = " . $id;
}
...
for ($i = 0; $i < count($result); $i++) {
    $queryFermate = "SELECT Fermate.* FROM Fermate INNER JOIN Fermate_Linee ON Fermate_Linee.idFermata = Fermate.id INNER JOIN Linee ON Fermate_Linee.idLinea = Linee.id WHERE Linee.id = " . $result[$i]["id"];
